...
def apply_power_scores(fighters, weights):
    ...
    for fighter in fighters:
        maj = int(fighter.get("major_months") or 0)
    ...
    ws = [weights["avg_elo"], weights["wtitle_months"], weights["event_wins"], weights["win_pct"], weights["sos"]]
    for col in ps_cols:
...
def get_all_season_power_scores():
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        f_h = pool.submit(select_view_dicts, "SELECT Fighter_Name, Season, COALESCE(Months_With_Major, 0) AS major_months, COALESCE(Months_With_Title, 0) AS champ_months, " + ", ".join(f"`{col}`" for col in POWER_EVENT_COLS) + " FROM holistic_view")
