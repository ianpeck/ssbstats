...
- **`app.py`** — minimal entrypoint that exposes `app:app` for local runs and Gunicorn.
...
- **`templates/partials/`** — reusable sections for the largest pages.
- **`static/css/style.css`** — dark glassmorphism theme. CSS custom properties defined at top.
- **`static/js/app.js`** — shared JS helpers.
- **`static/js/pages/`** — page-specific JS extracted from templates.
- **`scripts/maintenance/`** — standalone local utilities like Elo recomputation and Power Score inspection.
- **`generated/`** — local output artifacts; not app source.
...
Fighter images live in `static/assets/fighters/` as PNGs. Names are generated by `fighter_to_filename()` in `ssbstats_app/utils.py`:
- lowercase, strip spaces/periods, replace `&` with `and`
- Special case: `"Banjo & Kazooie"` → `"banjoandkazooie"`

Stage images live in `static/assets/stages/`. Names via `stage_to_filename()` in `ssbstats_app/utils.py`: lowercase, strip spaces/commas/apostrophes/parens/hyphens.
...
- `serialize_value()` converts MySQL `Decimal` → `float` and dates → ISO strings for JSON responses
